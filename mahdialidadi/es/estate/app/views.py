...
class SellView(ListView):
	...
	def post(self,request,*args,**kwargs):
		if self.request.method == "POST":
			...
			mtype = "آپارتمان" if mtype == "apartment" else "اداری و تجاری" if mtype =="office" else "کلنگی" if mtype=="kolangi" else ".*"
...
class RentView(ListView):
	...
	def post(self,request,*args,**kwargs):
		if self.request.method == "POST":
			...
			area ="اکباتان" if area == "ekbatan" else "بیمه" if area =="bime" else ".*"
			mtype = request.POST.get("mtype")
			mtype = "آپارتمان" if mtype == "apartment" else "اداری و تجاری" if mtype =="office" else "ویلایی" if mtype =="villa" else ".*"
